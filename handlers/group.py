...
def timer(context):
    ...
    if len(data["players"]) >= 3:
        if database.get_new_player([all_player["user_id"] for all_player in data["players"]]):
            buttons = group_helpers.tutorial_button(get_string(lang, "yes"), get_string(lang, "no"))
            text = get_string(lang, "tutorial")
            context.bot.send_message(chat_id, text, reply_markup=buttons)
            dp.chat_data[chat_id].update({"lang": lang, "group_settings": data["group_settings"],
                                          "players": data["players"], "starter": data["starter"],
                                          "message": data["message"], "tutorial": {"voters": [], "yes": 0, "no": 0}})
        else:
...
def tutorial(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    lang = chat_data["lang"]
    if user_id in chat_data["tutorial"]["voters"]:
        query.answer(get_string(lang, "already_voted"))
        return
    chat_data["tutorial"][decision] += 1
    query.answer(get_string(lang, "voted"))
    buttons = group_helpers.tutorial_button(get_string(lang, "yes"), get_string(lang, "no"))
    text = get_string(lang, "tutorial") + "\n\n" + get_string(lang, "tutorial_voters")\
        .format(chat_data["tutorial"]["voters"]), len(chat_data["players"])
    if len(chat_data["tutorial"]["voters"]) / len(chat_data["players"]) >= 2:
        query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(buttons))
        return
    yes = chat_data["tutorial"]["yes"]
    no = chat_data["tutorial"]["no"]
    if yes == no:
        query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(buttons))
        return
    elif yes > no:
        chat_data["tutorial"] = True
    else:
        chat_data["tutorial"] = False
    query.edit_message_text(text)
    group_helpers.yes_game(context, chat_data, update.effective_chat.id, False)
