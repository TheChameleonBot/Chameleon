...
def yes_game(context, data, chat_id, dp):
    if dp:
        # that means we come from the job
        chat_data = dp.chat_data[chat_id]
    else:
        # that means we come from the tutorial inlinequery
...
def tutorial_button(yes, no):
    return [[InlineKeyboardButton(yes, callback_data="tutorial_yes"),
             InlineKeyboardButton(no, callback_data="tutorial_no")]]
